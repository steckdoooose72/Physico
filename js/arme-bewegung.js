/* ==========================================================================
   arme-bewegung.js – Pilotmodus „Bewegung" (Arme)
   --------------------------------------------------------------------------
   Zweites der Arme-Module (nach arme-struktur.js). Technisch identisch zu
   js/hals-bewegung.js/js/rumpf-bewegung.js aufgebaut – siehe dort für die
   ausführlichere Erklärung des Ablaufs.

   Eine Bewegung steht als Text da („Der Ellenbogen wird gebeugt"), und der
   Muskel, der sie ausführt, wird aus den Antworten des gewählten Umfangs
   gewählt.

   Der Umfang wird **vorab** gewählt, nicht pro Frage angezeigt:
     Einstieg    – die zwei eindeutigen Hauptbewegungen (Flexion, Extension)
     Vertiefung  – dazu zwei feinere Fragen zu Pronation und Supination

   Wichtiger fachlicher Punkt bei Frage 4 (Supination): der M. biceps
   brachii ist zwar ebenfalls ein Supinator, aber nur bei GEBEUGTEM
   Ellenbogen kräftig – der M. supinator wirkt dagegen unabhängig von der
   Ellenbogenstellung. Die Funktionszeile stellt diesen Unterschied
   ausdrücklich heraus, analog zum M. sternocleidomastoideus-Hinweis in
   js/hals-bewegung.js.
   ...
   Und wie in js/arme-struktur.js: nach der Antwort bleibt der richtige
...
const BEWEGUNGEN = [
  {
    umfang: 'klar',
    bewegung: 'Der Ellenbogen wird gebeugt (Flexion).',
  ...
  {
    umfang: 'klar',
    bewegung: 'Der Ellenbogen wird gestreckt (Extension).',
  ...
  {
    umfang: 'fein',
    bewegung: 'Der Unterarm wird gedreht, die Handfläche zeigt nach unten (Pronation).',
    name: 'Mm. pronatores',
    ids: ['FMA38561', 'FMA38563', 'FMA38455'],
    funktion: 'Drehen den Unterarm in Pronation, unabhängig von der Ellenbogenstellung.',
  },
  {
    umfang: 'fein',
    bewegung: 'Der Unterarm wird gedreht, die Handfläche zeigt nach oben (Supination).',
    name: 'M. supinator',
    ids: ['FMA38514'],
    funktion: 'Dreht den Unterarm in Supination, unabhängig von der Ellenbogenstellung – im '
              + 'Gegensatz zum M. biceps brachii, der nur bei gebeugtem Ellenbogen kräftig supiniert.',
  },
];

// Für das Skelett als ruhiger Hintergrund – lädt schnell, steht schon hinter
// der Umfangswahl.
const KNOCHEN_BUENDEL = ['skelett_arm'];
// Die Muskeln kommen erst nach der Wahl dazu (deutlich größer).
const MUSKEL_BUENDEL = ['muskeln_arm'];
...
async function starteUmfang(umfang) {
  ...
  kopfUmfang.textContent =
    umfang === 'einstieg' ? 'Arme · Einstieg' : 'Arme · Vertiefung';
...
try {
  ...
  console.log(`Arme-Bewegung: ${BEWEGUNGEN.filter((b) => b.umfang === 'klar').length} klar, `
