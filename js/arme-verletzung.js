/* ==========================================================================
   arme-verletzung.js – Pilotmodus „Verletzungsmechanismus" (Arme)
   --------------------------------------------------------------------------
   Drittes der Arme-Module (nach arme-struktur.js und arme-bewegung.js).
   Technisch identisch zu js/knie-verletzung.js/js/rumpf-verletzung.js
   ...
   („Sturz auf die ausgestreckte Hand …"), und die betroffene Struktur wird
   ...
   **nicht** vorwegnehmen. Sie zeigt während der Fragen nur neutral die
   Armregion; erst in der Auswertung lässt sich jede Struktur der Runde
   einzeln im Modell nachsehen (Klick auf die Liste).

   Kein Marker-Sonderfall nötig: alle vier Antwortstrukturen (Radius,
   Kahnbein, M. triceps brachii, Retinaculum flexorum) sind echte Meshes,
   keine reinen Koordinatenpunkte.
   ...
   (arme-verletzung.html), eigene Datendatei (daten/arme_verletzungen.json).
   ========================================================================== */
...
// gleich. Koordinate grob zwischen dem Trizeps (FMA37700, y≈1.19) und dem
// Retinaculum flexorum am Handgelenk (y≈0.81) aus daten/strukturen.json,
// als Mittelpunkt zwischen Ellenbogen und Handgelenk.
const ARME_MITTE = new THREE.Vector3(0.22, 1.0, -0.02);

// Wie viel Körper (in Metern) das Bild bei den Fragen zeigt – deckt
// Ellenbogen bis Handgelenk ab, mit etwas Luft für den Trizeps oben.
const SICHT_HOEHE = 0.55;

/**
 * Welcher Antwortbegriff steckt hinter welchen Strukturen im Modell. Die
 * Kennungen sind identisch zu denen in js/arme-struktur.js übernommen,
 * nicht neu nachgeschlagen.
 */
const STRUKTUR_KENNUNGEN = {
  'Radius': { ids: ['FMA23465'] },
  'Kahnbein': { ids: ['FMA24436'] },
  'M. triceps brachii': { ids: ['FMA37700', 'FMA37696', 'FMA37698'] },
...
async function ladeFaelle() {
  const antwort = await fetch('daten/arme_verletzungen.json');
  if (!antwort.ok) throw new Error('daten/arme_verletzungen.json fehlt.');
  const alle = await antwort.json();
  if (!Array.isArray(alle) || !alle.length) {
    throw new Error('daten/arme_verletzungen.json enthält keine Fälle.');
  }
  return alle;
}
...
/**
 * Rahmt die Armregion ein – bei jeder Frage derselbe Ausschnitt, damit
...
function rahmeArm() {
  ...
  // Blickpunkt unter den Arm legen, damit er über der Antwortfläche liegt
 ...
 * (schwierigkeit „klar" in daten/arme_verletzungen.json), „vertiefung" alle.
 */
...
try {
  ...
  console.log(`Arme-Verletzung: ${klar} klar, ${faelle.length} gesamt.`);
} catch (fehler) {
