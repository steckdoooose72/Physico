/* ==========================================================================
   beine-fall.js – Pilotmodus „Fallvignette" (Beine)
   --------------------------------------------------------------------------
   ...
   js/knie-fall.js, js/kopf-fall.js, js/hals-fall.js, js/rumpf-fall.js und
   js/arme-fall.js aufgebaut – siehe dort für die ausführlichere Erklärung
   des Ablaufs.
   ...
   Wie in js/beine-verletzung.js: die 3D-Szene verrät während der Fragen
   nichts, sie zeigt nur neutral die Beinregion. Erst in der Auswertung
   lässt sich jede beantwortete Teilfrage einzeln im Modell nachsehen –
   inklusive des Marker-Sonderfalls „Hüftgelenk"/„Oberes Sprunggelenk"/
   „Unteres Sprunggelenk" (reine Koordinatenpunkte, 0 Dreiecke, System
   „gelenkpunkte"), die wie in hals-fall.js/arme-fall.js über eine
   leuchtende Kugel dargestellt werden. Alle anderen Antwortstrukturen sind
   echte Meshes (Knochen, Muskeln oder die schematischen Bänder) und werden
   normal eingefärbt.

   Hinweis zur Struktur-Benennung: „Mm. glutei" fasst hier – wie in
   js/beine-struktur.js – alle drei Gesäßmuskel-Anteile zu einer Gruppe
   zusammen (FMA22329, FMA22331, FMA22333). js/beine-bewegung.js/
   js/beine-alltag.js führen stattdessen zwei getrennte Strukturen
   („M. gluteus maximus" bzw. „M. gluteus medius/minimus") – für die
   Fallvignette gilt die STRUKTUR-Liste aus beine-struktur.js, deshalb hier
   die zusammengefasste Gruppe.

   Pilot: absichtlich nicht in der Navigation verlinkt, eigene Seite
   (beine-fall.html), eigene Datendatei (daten/beine_faelle.json).
   ========================================================================== */
...
// gleich. Gleiche Koordinate und Sichthöhe wie js/beine-verletzung.js.
 ...
 * (0 Dreiecke, System „gelenkpunkte") – dafür wird wie in
 * js/beine-struktur.js eine kleine leuchtende Kugel an die „mitte"-
 * Koordinate gesetzt, statt ein Mesh einzufärben.
 */
const STRUKTUR_KENNUNGEN = {
  ...
  'Hüftgelenk': { ids: ['PT-G-huefte-links'], marker: true },
  'Oberes Sprunggelenk': { ids: ['PT-G-osg-links'], marker: true },
  'Unteres Sprunggelenk': { ids: ['PT-G-usg-links'], marker: true },
  'Mm. glutei': { ids: ['FMA22329', 'FMA22331', 'FMA22333'] },
  'M. iliopsoas': { ids: ['FMA22343', 'FMA22323'] },
  'Mm. adductores': { ids: ['FMA22460', 'FMA22457', 'FMA22454', 'FMA43884', 'FMA22451'] },
  'M. tibialis posterior': { ids: ['FMA65019'] },
  'M. tibialis anterior': { ids: ['FMA22545'] },
  'Mm. fibulares': { ids: ['FMA22553', 'FMA22555'] },
};
 ...
 * wiederverwendet und je nach Bedarf umgesetzt – die Gelenkpunkte selbst
 * haben keine Geometrie, die man einfärben könnte. Gleicher Aufbau wie in
 * js/beine-struktur.js.
 */
...
async function ladeFaelle() {
  const antwort = await fetch('daten/beine_faelle.json');
  if (!antwort.ok) throw new Error('daten/beine_faelle.json fehlt.');
  const alle = await antwort.json();
  if (!Array.isArray(alle) || !alle.length) {
    throw new Error('daten/beine_faelle.json enthält keine Fälle.');
  }
  return alle;
}
 ...
 * gleiche Rechnung wie in js/beine-verletzung.js.
 */
...
/**
 * Rahmt die Beinregion ein – bei jedem Fall derselbe Ausschnitt, damit die
 * Kamera nichts verrät. Bewusst von Hand gerechnet statt über fliegeZu() –
 * siehe js/beine-verletzung.js für die Begründung.
 */
function rahmeBein() {
 ...
 * Verzweigt auf den Marker (Hüftgelenk/Oberes/Unteres Sprunggelenk) oder
 * auf normale Mesh-Einfärbung mit Bündel-Nachladen, wie in
 * beine-struktur.js bzw. beine-verletzung.js.
 */
async function hebeHervor(name, knopf) {
  ...
  if (eintrag.marker) {
    ...
    // vom Bündel-Ladezustand – wie in js/beine-struktur.js.
...
try {
  ...
  console.log(`Beine-Fall: ${faelle.length} Fälle, `
