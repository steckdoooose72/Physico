/* ==========================================================================
   hals-bewegung.js – Pilotmodus „Bewegung" (Hals)
   --------------------------------------------------------------------------
   Zweites der Hals-Module (nach hals-struktur.js). Technisch identisch zu
   js/knie-bewegung.js aufgebaut – siehe dort für die ausführlichere
   Erklärung des Ablaufs.

   Eine Bewegung steht als Text da („Der Kopf wird nach vorn gebeugt"), und
   der Muskel, der sie ausführt, wird aus vier Antworten gewählt.

   Der Umfang wird **vorab** gewählt, nicht pro Frage angezeigt:
     Einstieg    – die zwei eindeutigen Hauptbewegungen (Flexion, Extension)
     Vertiefung  – dazu zwei feinere Fragen zu Rotation und Lateralflexion

   Wichtiger fachlicher Punkt bei Frage 3 (Rotation): der M. sternocleido-
   mastoideus dreht den Kopf zur GEGENSEITE – der LINKE Muskel dreht nach
   RECHTS. Die Antwort auf „Kopf dreht sich nach rechts" ist deshalb der
   linke SCM, und genau der wird hier auch als Mesh hervorgehoben (die
   Muskeln liegen ohnehin nur links vor, wie in den anderen Piloten – hier
   ist die Gegenseitigkeit aber der eigentliche Lerninhalt der Frage, nicht
   nur ein Nebenaspekt der Seitenwahl).
   ...
   Und wie in js/hals-struktur.js: nach der Antwort bleibt der richtige
...
const BEWEGUNGEN = [
  {
    umfang: 'klar',
    bewegung: 'Der Kopf wird nach vorn gebeugt, das Kinn zur Brust geführt (Flexion).',
    name: 'Prävertebrale Muskulatur',
    ids: ['FMA46310', 'FMA46288', 'FMA46284', 'FMA46286'],
    funktion: 'Tiefe, vordere Halsmuskulatur, beugt den Kopf nach vorn.',
  },
  {
    umfang: 'klar',
    bewegung: 'Der Kopf wird nach hinten gebeugt (Extension).',
    name: 'Mm. splenius capitis/cervicis',
    ids: ['FMA22729', 'FMA22727'],
    funktion: 'Dorsale Halsmuskeln, strecken den Kopf nach hinten.',
  },
  {
    umfang: 'fein',
    bewegung: 'Der Kopf wird zur rechten Seite gedreht (Rotation).',
    name: 'M. sternocleidomastoideus',
    ids: ['FMA13409'],
    funktion: 'Dreht den Kopf zur GEGENSEITE – der linke Muskel dreht nach rechts, der rechte nach links. Beidseitige Kontraktion beugt den Kopf nach vorn.',
  },
  {
    umfang: 'fein',
    bewegung: 'Der Kopf wird seitlich zur Schulter geneigt (Lateralflexion).',
    name: 'Mm. scaleni',
    ids: ['FMA13393', 'FMA13391', 'FMA13389'],
    funktion: 'Seitliche Halsmuskeln, neigen den Hals zur gleichen Seite und unterstützen die Einatmung.',
  },
];

// Für das Skelett als ruhiger Hintergrund – lädt schnell, steht schon hinter
// der Umfangswahl. Atlas/Axis liegen im Kopf-Bündel, die übrigen
// Halswirbel im Hals-Bündel.
const KNOCHEN_BUENDEL = ['skelett_kopf', 'skelett_hals'];
// Die Muskeln kommen erst nach der Wahl dazu (deutlich größer). M. longus
// capitis und ein Teil des M. longus colli liegen im Kopf-Bündel, der Rest
// im Hals-Bündel.
const MUSKEL_BUENDEL = ['muskeln_kopf', 'muskeln_hals'];
...
async function starteUmfang(umfang) {
  ...
  kopfUmfang.textContent =
    umfang === 'einstieg' ? 'Hals · Einstieg' : 'Hals · Vertiefung';
...
try {
  ...
  console.log(`Hals-Bewegung: ${BEWEGUNGEN.filter((b) => b.umfang === 'klar').length} klar, `
