/* ==========================================================================
   hals-verletzung.js – Pilotmodus „Verletzungsmechanismus" (Hals)
   --------------------------------------------------------------------------
   Drittes der Hals-Module (nach hals-struktur.js und hals-bewegung.js).
   ...
   („Auffahrunfall …"), und die betroffene Struktur wird aus mehreren
   Antworten gewählt.
   ...
   **nicht** vorwegnehmen. Sie zeigt während der Fragen nur neutral die
   Halsregion; erst in der Auswertung lässt sich jede Struktur der Runde
   einzeln im Modell nachsehen (Klick auf die Liste).

   Kein Marker-Sonderfall nötig: alle drei Antwortstrukturen (SCM, Mm.
   splenius, Dens-Stabilisierungsbänder) sind echte Meshes – die
   Dens-Stabilisierungsbänder sind zwar ein schematisches Pfad-Mesh (System
   „baender"), werden aber wie Knochen/Muskeln normal eingefärbt, genau wie
   der Discus articularis in js/kopf-verletzung.js.
   ...
   (hals-verletzung.html), eigene Datendatei (daten/hals_verletzungen.json).
   ========================================================================== */
...
// als grober Mittelpunkt der Halswirbelsäule.
const HALS_MITTE = new THREE.Vector3(0, 1.43, -0.02);

// Wie viel Körper (in Metern) das Bild bei den Fragen zeigt – groß genug für
// die ganze Halsregion von der Schädelbasis bis zu den Schultern, klein
// genug, um nicht abzulenken.
 ...
 * js/hals-bewegung.js übernommen, nicht neu nachgeschlagen. Kein Marker
...
// Hals-Bündel. Die Dens-Stabilisierungsbänder kommen upfront mit dazu
// (siehe unten), die Muskeln erst bei Bedarf (siehe hebeHervor()).
...
async function ladeFaelle() {
  const antwort = await fetch('daten/hals_verletzungen.json');
  if (!antwort.ok) throw new Error('daten/hals_verletzungen.json fehlt.');
  const alle = await antwort.json();
  if (!Array.isArray(alle) || !alle.length) {
    throw new Error('daten/hals_verletzungen.json enthält keine Fälle.');
  }
  return alle;
}
 ...
 *
 * Die Dens-Stabilisierungsbänder bleiben hier aus, obwohl geladen: sie
 * würden sonst schon vor der Antwort verraten, worum es in der Frage geht.
 */
...
/**
 * Rahmt die Halsregion ein – bei jeder Frage derselbe Ausschnitt, damit die
 * Kamera nichts verrät.
 *
 * Bewusst von Hand gerechnet statt über fliegeZu() – siehe
 * js/kopf-verletzung.js für die Begründung (Gelenkpunkt-Mesh trägt
 * unsichtbare Bewegungsachsen als Kinder, die die Hüllbox aufblähen würden).
 */
function rahmeHals() {
  ...
  // Blickpunkt unter den Hals legen, damit er über der Antwortfläche liegt
 ...
 * (schwierigkeit „klar" in daten/hals_verletzungen.json), „vertiefung" alle.
 */
...
try {
  ...
  await katalog.ladeBuendel('eigene');   // Dens-Stabilisierungsbänder, bleiben zunächst aus
  ...
  console.log(`Hals-Verletzung: ${klar} klar, ${faelle.length} gesamt.`);
} catch (fehler) {
