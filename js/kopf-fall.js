/* ==========================================================================
   kopf-fall.js – Pilotmodus „Fallvignette" (Kopf)
   --------------------------------------------------------------------------
   ...
   Aspekten desselben Falls. Technisch identisch zu js/schulter-fall.js und
   js/knie-fall.js aufgebaut – siehe dort für die ausführlichere Erklärung
   des Ablaufs.
   ...
   Wie in js/kopf-verletzung.js: die 3D-Szene verrät während der Fragen
   nichts, sie zeigt nur neutral die Kieferregion. Erst in der Auswertung
   lässt sich jede beantwortete Teilfrage einzeln im Modell nachsehen –
   inklusive des Marker-Sonderfalls „Kiefergelenk" (reiner Koordinatenpunkt,
   0 Dreiecke, System „gelenkpunkte"), der wie in kopf-struktur.js über eine
   leuchtende Kugel dargestellt wird. „Discus articularis" ist dagegen ein
   echtes, wenn auch schematisches Pfad-Mesh und wird ganz normal
   eingefärbt.

   Pilot: absichtlich nicht in der Navigation verlinkt, eigene Seite
   (kopf-fall.html), eigene Datendatei (daten/kopf_faelle.json).
   ========================================================================== */
...
const STRUKTUR_KENNUNGEN = {
  ...
  'M. temporalis':                { ids: ['FMA49008'] },
  'M. pterygoideus medialis':     { ids: ['FMA49013'] },
  'M. pterygoideus lateralis':    { ids: ['FMA49023', 'FMA49025'] },
 ...
 * könnte. Gleicher Aufbau wie in js/kopf-struktur.js.
 */
...
async function ladeFaelle() {
  const antwort = await fetch('daten/kopf_faelle.json');
  if (!antwort.ok) throw new Error('daten/kopf_faelle.json fehlt.');
  const alle = await antwort.json();
  if (!Array.isArray(alle) || !alle.length) {
    throw new Error('daten/kopf_faelle.json enthält keine Fälle.');
  }
  return alle;
}
...
/**
 * Rahmt die Kieferregion ein – bei jedem Fall derselbe Ausschnitt, damit
 * die Kamera nichts verrät. Bewusst von Hand gerechnet statt über
 * fliegeZu() – siehe js/kopf-verletzung.js für die Begründung
 ...
 * mit Bündel-Nachladen, wie in kopf-struktur.js bzw. kopf-verletzung.js.
 */
...
try {
  ...
  console.log(`Kopf-Fall: ${faelle.length} Fälle, `
