/* ==========================================================================
   ...
   genug und deshalb hier nicht Teil des Pools. Technisch identisch zu
   js/schulter-struktur.js/js/knie-struktur.js aufgebaut – siehe dort für
 ...
 * zu sehen ist (siehe hebeHervor()). Das betrifft hier nur das Kiefergelenk
 * selbst – der Discus articularis (System „baender") ist dagegen ein echtes,
 * wenn auch schematisches Pfad-Mesh und wird wie Knochen/Muskeln behandelt.
 */
const STRUKTUREN = [
  {
    name: 'Unterkiefer',
 ...
 * und verdecken die Hervorhebung, der Discus articularis würde schon vor der
 ...
 * Muskeln, aber auch die schematische Struktur – für die Hervorhebung macht
 * das keinen Unterschied, sie ist genauso ein reales Mesh) oder – beim
 * reinen Koordinatenpunkt Kiefergelenk – den Marker an seine Stelle setzen.
...
try {
  ...
  for (let i = 0; i < BENOETIGTE_BUENDEL.length; i++) {
    status(`Kopf wird geladen … (${i + 1}/${BENOETIGTE_BUENDEL.length})`);
  ...
  console.log(`Kopf-Struktur: ${STRUKTUREN.length} Strukturen im Pool.`);
} catch (fehler) {
