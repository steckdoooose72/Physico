/* ==========================================================================
   kopf-verletzung.js – Pilotmodus „Dysfunktion" (Kopf)
   --------------------------------------------------------------------------
   Drittes der Kopf-Module (nach kopf-struktur.js und kopf-bewegung.js).
   ...
   („Nächtliches Zähneknirschen …"), und die betroffene Struktur wird aus
   mehreren Antworten gewählt.

   Andere Bezeichnung als bei Schulter/Knie: „Dysfunktion" statt
   „Verletzungsmechanismus" – beim Kiefergelenk geht es seltener um ein
   akutes Trauma, meist um funktionelle Überlastung (Bruxismus,
   Diskusverlagerung). Technisch ist der Ablauf identisch, nur Titel und
   Einleitungstext sind angepasst.
   ...
   **nicht** vorwegnehmen. Sie zeigt während der Fragen nur neutral die
   Kieferregion; erst in der Auswertung lässt sich jede Struktur der Runde
   einzeln im Modell nachsehen (Klick auf die Liste).

   Anders als bei knie-verletzung.js kommt hier der Marker-Sonderfall aus
   kopf-struktur.js wieder vor: „Kiefergelenk" ist ein reiner
   Koordinatenpunkt (0 Dreiecke, System „gelenkpunkte") und wird deshalb wie
   dort über eine leuchtende Kugel dargestellt, nicht über Mesh-Einfärbung.
   „Discus articularis" ist dagegen ein echtes, wenn auch schematisches
   Pfad-Mesh (System „baender") und wird ganz normal eingefärbt.
   ...
   (kopf-verletzung.html), eigene Datendatei (daten/kopf_verletzungen.json).
   ========================================================================== */
...
// Das linke Kiefergelenk ist die Pilotseite – eine Seite genügt für einen
// klinischen Fall, und die Kamera steht dadurch bei jeder Frage gleich.
// Koordinate = „mitte" von PT-G-kiefer-links aus daten/strukturen.json.
const KIEFER_MITTE = new THREE.Vector3(0.049, 1.5181, -0.0083);

// Wie viel Körper (in Metern) das Bild bei den Fragen zeigt – die Kiefer-
// region ist deutlich kompakter als Schulter oder Knie, deshalb entsprechend
// klein gewählt (Masseter ca. 0.076 m groß).
const SICHT_HOEHE = 0.16;

/**
 * Welcher Antwortbegriff steckt hinter welchen Strukturen im Modell. Die
 * Kennungen sind identisch zu denen in js/kopf-struktur.js übernommen,
 * nicht neu nachgeschlagen. `marker: true` heißt: reiner Koordinatenpunkt
 * (0 Dreiecke, System „gelenkpunkte") – dafür wird wie in kopf-struktur.js
 * eine kleine leuchtende Kugel an die „mitte"-Koordinate gesetzt.
 */
const STRUKTUR_KENNUNGEN = {
  'M. masseter':        { ids: ['FMA49002', 'FMA49005'] },
  'Discus articularis': { ids: ['PT-B-discus-links'] },
  'Kiefergelenk':       { ids: ['PT-G-kiefer-links'], marker: true },
};

// Knöcherner Rahmen um den Kiefer: reicht als ruhiger Hintergrund und lädt
// schnell. Der Discus articularis und der Kiefergelenk-Koordinatenpunkt
// kommen upfront mit dazu (siehe unten), die Muskeln erst bei Bedarf (siehe
// hebeHervor()).
const GRUND_BUENDEL = ['skelett_kopf'];
...
// --- Bühne ----------------------------------------------------------------
// Blick fest aufs linke Kiefergelenk: bei jeder Frage derselbe Ausschnitt,
...
const { szene, kamera, steuerung, fliegeZu, schattenAuffrischen, starteSchleife } =
  erzeugeBuehne({
    ...
    kameraStart: new THREE.Vector3(0.15, 1.58, 0.12),
    blickZiel: KIEFER_MITTE.clone(),
    mitBoden: false,
  });
...
/**
 * Der Marker für den Kiefergelenk-Koordinatenpunkt: eine kleine leuchtende
 * Kugel in der Hervorhebungsfarbe. Wird einmal gebaut und bei Bedarf an die
 * richtige Koordinate gesetzt – der Gelenkpunkt selbst hat keine Geometrie,
 * die man einfärben könnte. Gleicher Aufbau wie in js/kopf-struktur.js.
 */
...
async function ladeFaelle() {
  const antwort = await fetch('daten/kopf_verletzungen.json');
  if (!antwort.ok) throw new Error('daten/kopf_verletzungen.json fehlt.');
  const alle = await antwort.json();
  if (!Array.isArray(alle) || !alle.length) {
    throw new Error('daten/kopf_verletzungen.json enthält keine Fälle.');
  }
  return alle;
}
 ...
 * Hinweis.
 *
 * Der Discus articularis bleibt hier aus, obwohl geladen: er würde sonst
 * schon vor der Antwort verraten, worum es in der Frage geht.
 */
...
/**
 * Kopfzeile und Antwortfläche liegen über der Bühne. Damit der Kiefer nicht
...
/**
 * Rahmt die Kieferregion ein – bei jeder Frage derselbe Ausschnitt, damit
...
function rahmeKiefer() {
  ...
  // Blickpunkt unter den Kiefer legen, damit er über der Antwortfläche liegt
  const ziel = KIEFER_MITTE.clone().addScaledVector(bildOben, -versatzHoch * SICHT_HOEHE);
 ...
 * Verzweigt auf den Marker (Kiefergelenk) oder auf normale Mesh-Einfärbung
 * mit Bündel-Nachladen, wie in kopf-struktur.js bzw. knie-verletzung.js.
 */
async function hebeHervor(name, knopf) {
  ...
  if (eintrag.marker) {
    ...
    // vom Bündel-Ladezustand – wie in js/kopf-struktur.js.
 ...
 * (schwierigkeit „klar" in daten/kopf_verletzungen.json), „vertiefung" alle.
 */
...
try {
  ...
  for (let i = 0; i < GRUND_BUENDEL.length; i++) {
    status(`Kiefer wird geladen … (${i + 1}/${GRUND_BUENDEL.length})`);
    await katalog.ladeBuendel(GRUND_BUENDEL[i]);
  }
  await katalog.ladeBuendel('eigene');   // Discus articularis + Gelenkpunkt, bleiben zunächst aus
  ...
  console.log(`Kopf-Verletzung: ${klar} klar, ${faelle.length} gesamt.`);
} catch (fehler) {
