/* ==========================================================================
   rumpf-bewegung.js – Pilotmodus „Bewegung" (Rumpf)
   --------------------------------------------------------------------------
   Zweites der Rumpf-Module (nach rumpf-struktur.js). Technisch identisch zu
   js/knie-bewegung.js aufgebaut – siehe dort für die ausführlichere
   Erklärung des Ablaufs.

   Eine Bewegung steht als Text da („Der Rumpf wird nach vorn gebeugt"), und
   ...
   Wichtiger fachlicher Punkt bei Frage 3 (Rotation): anders als bei einem
   einzelnen Muskel arbeiten hier immer zwei Muskeln UNTERSCHIEDLICHER
   Seiten zusammen (z. B. rechter M. obliquus internus + linker M. obliquus
   externus für eine Drehung nach rechts) – analog zum M. sternocleido-
   mastoideus in js/hals-bewegung.js, nur mit zwei verschiedenen Muskeln
   statt einem. Die Funktionszeile stellt das ausdrücklich heraus. Gezeigt
   wird trotzdem nur die linke Seite (wie in allen anderen Piloten), da im
   Modell ohnehin nur eine Seite als Antwortoption dient und die
   Gegenseiten-Zusammenarbeit rein textlich vermittelt wird.
   ...
   Und wie in js/rumpf-struktur.js: nach der Antwort bleibt der richtige
...
const BEWEGUNGEN = [
  {
    umfang: 'klar',
    bewegung: 'Der Rumpf wird nach vorn gebeugt (Flexion).',
  ...
  {
    umfang: 'klar',
    bewegung: 'Der Rumpf wird aus einer gebeugten Position wieder aufgerichtet (Extension).',
    name: 'M. multifidus',
    ids: ['FMA22879'],
    funktion: 'Tiefer, segmentaler Rückenmuskel, stabilisiert einzelne Wirbelsegmente und richtet den Rumpf auf.',
  },
  {
    umfang: 'fein',
    bewegung: 'Der Rumpf wird zur Seite gedreht (Rotation).',
    name: 'Mm. obliqui abdominis',
    ids: ['FMA13337', 'FMA13893'],
    funktion: 'Arbeiten mit der Gegenseite zusammen – z. B. innerer schräger Bauchmuskel derselben Seite + äußerer schräger Bauchmuskel der Gegenseite für eine Drehung in eine Richtung.',
  },
  {
    umfang: 'fein',
    bewegung: 'Der Rumpf wird seitlich zur Hüfte geneigt (Lateralflexion).',
    name: 'M. quadratus lumborum',
    ids: ['FMA22349'],
    funktion: 'Seitlicher Lendenmuskel, neigt den Rumpf zur gleichen Seite.',
  },
];

// Für das Skelett als ruhiger Hintergrund – lädt schnell, steht schon hinter
// der Umfangswahl. Die obersten Brustwirbel/-rippen liegen im Hals-Bündel,
// der Rest von Wirbelsäule und Rippen im Rumpf-Bündel.
const KNOCHEN_BUENDEL = ['skelett_hals', 'skelett_rumpf'];
// Die Muskeln kommen erst nach der Wahl dazu (deutlich größer).
const MUSKEL_BUENDEL = ['muskeln_rumpf'];
...
try {
  ...
  console.log(`Rumpf-Bewegung: ${BEWEGUNGEN.filter((b) => b.umfang === 'klar').length} klar, `
