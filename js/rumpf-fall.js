/* ==========================================================================
   rumpf-fall.js – Pilotmodus „Fallvignette" (Rumpf)
   --------------------------------------------------------------------------
   ...
   js/knie-fall.js, js/kopf-fall.js und js/hals-fall.js aufgebaut – siehe
   ...
   Wie in js/rumpf-verletzung.js: die 3D-Szene verrät während der Fragen
   nichts, sie zeigt nur neutral die Rumpfregion. Erst in der Auswertung
   lässt sich jede beantwortete Teilfrage einzeln im Modell nachsehen.

   Kein Marker-Sonderfall nötig: alle Antwortstrukturen dieser beiden Fälle
   (M. multifidus, Bandscheibe (BWS/LWS), Lendenwirbel, M. rectus abdominis,
   Kreuzbein, M. quadratus lumborum, Beckenboden, Rippe, Brustwirbel,
   Brustbein, Hüftbein, Diaphragma, Mm. obliqui abdominis) sind echte
   Meshes, keine reinen Koordinatenpunkte – anders als das Iliosakralgelenk
   in js/rumpf-struktur.js, das hier gar nicht vorkommt.

   Fall 2/Teilfrage 3: „M. transversus abdominis" steht NICHT in der für
   dieses Modul vorgegebenen Struktur-Liste (siehe rumpf-struktur.js/
   rumpf-bewegung.js) – als Ablenker wurden deshalb stattdessen
   M. rectus abdominis und Mm. obliqui abdominis verwendet.

   Pilot: absichtlich nicht in der Navigation verlinkt, eigene Seite
   (rumpf-fall.html), eigene Datendatei (daten/rumpf_faelle.json).
   ========================================================================== */
...
// gleich. Gleiche Koordinate und Sichthöhe wie js/rumpf-verletzung.js.
 ...
 * Kennungen sind identisch zu denen in js/rumpf-struktur.js/
 * js/rumpf-bewegung.js übernommen, nicht neu nachgeschlagen.
 */
const STRUKTUR_KENNUNGEN = {
  ...
  'M. rectus abdominis': { ids: ['FMA13378'] },
  'Kreuzbein': { ids: ['FMA16202'] },
  'M. quadratus lumborum': { ids: ['FMA22349'] },
  'Beckenboden': { ids: ['FMA45855', 'FMA45859', 'FMA46444'] },
  'Rippe': {
    ids: [
  ...
  'Brustbein': { ids: ['FMA7486', 'FMA7487', 'FMA7488'] },
  'Hüftbein': { ids: ['FMA16587'] },
  'Diaphragma': { ids: ['FMA13295'] },
  'Mm. obliqui abdominis': { ids: ['FMA13337', 'FMA13893'] },
};
...
// im Rumpf-Bündel. Die Muskeln (Rumpf und Becken) kommen erst bei Bedarf
// dazu (siehe hebeHervor()).
...
async function ladeFaelle() {
  const antwort = await fetch('daten/rumpf_faelle.json');
  if (!antwort.ok) throw new Error('daten/rumpf_faelle.json fehlt.');
  const alle = await antwort.json();
  if (!Array.isArray(alle) || !alle.length) {
    throw new Error('daten/rumpf_faelle.json enthält keine Fälle.');
  }
  return alle;
}
 ...
 * Teilfrage-Text allein reicht hier NICHT: „Welche Struktur ist direkt/am
 * ehesten betroffen?" kommt in beiden Fällen leicht abgewandelt vor, mit
 ...
 * gleiche Rechnung wie in js/rumpf-verletzung.js.
 */
...
/**
 * Rahmt die Rumpfregion ein – bei jedem Fall derselbe Ausschnitt, damit die
 * Kamera nichts verrät. Bewusst von Hand gerechnet statt über fliegeZu() –
 * siehe js/rumpf-verletzung.js für die Begründung.
 */
function rahmeRumpf() {
 ...
 * Bündel der gesuchten Strukturen bei Bedarf nachladen, wie in
 * js/rumpf-verletzung.js.
 */
...
try {
  ...
  console.log(`Rumpf-Fall: ${faelle.length} Fälle, `
