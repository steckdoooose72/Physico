 ...
 * hier etwas zu sehen ist (siehe setzeMarker()).
 */
const STRUKTUREN = [
  {
    name: 'Schlüsselbein',
...
/**
 * Der Marker für die beiden Gelenkpunkte: eine kleine leuchtende Kugel in der
 * Hervorhebungsfarbe. Sie wird einmal gebaut und je Frage an die richtige
 * Koordinate gesetzt – die Gelenkpunkte selbst haben keine Geometrie, die man
 * einfärben könnte.
 */
 ...
 * Die Muskeln sind zwar geladen, bleiben aber unsichtbar – sie legen sich
 * sonst als milchige Schicht über alles und verdecken die Hervorhebung.
 */
...
/**
 * Hebt die gesuchte Struktur hervor: entweder die Meshes einfärben oder – bei
 * den reinen Koordinatenpunkten – den Marker an ihre Stelle setzen.
...
try {
  ...
  for (let i = 0; i < BENOETIGTE_BUENDEL.length; i++) {
    status(`Schulter wird geladen … (${i + 1}/${BENOETIGTE_BUENDEL.length})`);
